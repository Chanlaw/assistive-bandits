...
class mab_VG(VariantGenerator):
    ...
    def human_wrapper(self):
        # return ['teleop']#human_wrapper_dict.keys()
        return ['preemptive']
...
def run_task(v, num_cpu=8, log_dir="./data", ename=None,  **kwargs):
    ...
    if log_dir is not None:
        log_dir = osp.join(log_dir, v["human_policy"]+str(v["epsilon"])+"_"+str(v["n_episodes"]))
    ...
    pi_H = human_policy_dict[v["human_policy"]](bandit, epsilon=v["epsilon"])
    ...
    with sess.as_default():
        ...
        logger.log("-------------------")
        logger.log("Evaluating against {} with {} particles".format(pi_H.__class__.__name__, 
                                                                    pi_H.epsilon))
        logger.log("-------------------")
        ...
        for t in range(v["n_episodes"]):
            ...
            logger.record_tabular("Tested Against", pi_H.__class__.__name__)
        ...
        test_env = h_wrapper(bandit, pi_H, penalty=0.)
    ...
    if local_test:
        ...
        for v in variants:
            ...
            run_experiment_lite(run_task, 
                                ...
                                exp_name=v["human_policy"]+str(v["epsilon"]), 
                                variant=v)
    else:
        vg = mab_VG()
        variants = vg.variants()
